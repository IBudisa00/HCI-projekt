...
export default function MoviePreview({tvshow}){
  const myShow=tvshow[0];
    return(
        <div className='bg-gray-100 flex flex-col'>
            <NextSeo title={myShow.name}/>
            <Header/>
            <div className="grow ">
                ...
                <div className='flex bg-gray-100 py-6 flex-col md:flex-row'>
                    ...
                    <Image
                        ...
                        <div className='pt-6 pb-2 mb-2 pl-10 pr-5 w-72 sm:w-[600px] md:w-[750px] xl:w-[1200px]'>
                            <h1 className='text-2xl'>{myShow.name}</h1>
                            <div className='flex flex-row items-center'>
                                <p className='text-xl pr-2'>Rating:</p>
                                <h3 className='pr-1 text-xl'>{myShow.rate}%</h3>
                                <Image
                                    src={popcorn}
                                    layout='fixed'
                                    alt="popcorn"
                            ...
                            <p className='text-base pb-7'>{myShow.category}</p>
                            <div>
                                <p className='text-base pr-6 pt-2 text-justify'>{myShow.info}</p>
                            </div>
                        </div>
                </div>
...
export const getStaticPaths = () => {
  ...
  export const getStaticProps = (context) => {
    ...
    const tvshow = tvshows.filter((tvshow) => tvshow.id == id);
    //console.log(movie)
